...
public class PhoneGuy : MonoBehaviour
{
    ...
    {
        if (enableDebugLogs)
            Debug.Log("PhoneGuy: Hra ukončena");
        
        // Zastav nahrávku při ukončení noci pokud je to povoleno
        if (stopOnNightEnd && isPlaying)
        {
            StopRecording();
        }
    ...
    private System.Collections.IEnumerator ManageMuteButtonTiming()
    {
        ...
        // Zobraz MUTE tlačítko (pouze pokud se nahrávka přehrává)
        if (isPlaying)
        {
            ShowMuteButton();
        }
        ...
        {
            if (enableDebugLogs)
                Debug.Log("PhoneGuy: MUTE tlačítko nebylo zobrazeno - nahrávka se nepřehrává");
            yield break;
        }
        ...
        // Automaticky skryj MUTE tlačítko
    ...
    private System.Collections.IEnumerator StopWithFadeOut()
    {
        ...
        {
            phoneGuyAudioSource.Stop();
            phoneGuyAudioSource.volume = 1f; // Reset volume pro příští přehrání
        }
