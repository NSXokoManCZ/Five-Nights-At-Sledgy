...
public class Svetlo_Strop_Kancelar_Tlacitko : MonoBehaviour
{
    ...
    [SerializeField] private NightShiftManagement nightShiftManagement; // Reference na night shift management pro kontrolu menu
    
    [Header("Additional Lighting System")]
    [SerializeField] private GameObject additionalLightsParent; // Rodičovský objekt pro dodatečná světla (volitelné - pokud chcete přímé ovládání)
    ...
    private void OnMouseDown()
    {
        ...
        // Zkontroluj zda není aktivní main menu nebo custom night panel
        if (nightShiftManagement != null && nightShiftManagement.IsInMainMenu())
        {
            Debug.Log("Světlo tlačítko: Zablokováno - hra není aktivní (main menu/custom night panel)");
            return;
        }
        ...
        if (svetlo != null)
        {
            ...
            if (wasLightOn) // Pokud bylo světlo ZAPNUTÉ (nyní se vypnulo)
            {
                ...
                // Vypni dodatečná světla (pokud je přímé ovládání nastaveno)
            ...
            else // Pokud bylo světlo VYPNUTÉ (nyní se zapnulo)
            {
                ...
                // Zapni dodatečná světla (pokud je přímé ovládání nastaveno)
                if (additionalLightsParent != null)
                {
    ...
    void Start()
    {
        ...
        if (laserDoorsSystem == null)
        {
            laserDoorsSystem = FindFirstObjectByType<VentilaceLaseryDvere>();
            if (laserDoorsSystem != null)
            {
    ...
    public void SilentResetPower()
    {
        if (svetlo != null)
        {
            ...
            // Tiše zapni dodatečná světla (pokud je přímé ovládání nastaveno)
            if (additionalLightsParent != null)
            {
                additionalLightsParent.SetActive(true);
            }
    ...
    private void InitializeDoorButtons()
    {
        ...
        tlacitka_dvere_kancelar[] foundButtons = FindObjectsByType<tlacitka_dvere_kancelar>(FindObjectsSortMode.None);
    ...
    public void TriggerPowerOutage()
    {
        if (svetlo != null && svetlo.IsLightOn)
        {
            ...
            // Vypni dodatečná světla při výpadku elektřiny (pokud je přímé ovládání nastaveno)
            if (additionalLightsParent != null)
            {
                additionalLightsParent.SetActive(false);
            }
