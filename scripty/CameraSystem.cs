...
public class CameraSystem : MonoBehaviour
{
    ...
    [SerializeField] private Color disabledButtonColor = Color.red; // Barva tlačítka CamsOn při deaktivaci (během přesunu / v Utility Room)
    ...
    [SerializeField] private ThatcherAI thatcherAI; // Reference na Thatcher AI systém
    [SerializeField] private CameraPositionController cameraPositionController; // Reference na controller pro kontrolu přesunu mezi místnostmi

    private Coroutine currentBlinkCoroutine; // Reference na aktuální blikání
    private bool isButtonDisabledDuringMovement = false; // Sledování stavu tlačítka během přesunu
    ...
    void Update()
    {
        // Kontrola přesunu mezi místnostmi a úprava tlačítka CamsOn
        if (cameraPositionController != null && OpenCamerasButton != null && openCamerasButtonImage != null)
        {
            // Tlačítko je deaktivované během přesunu NEBO když jsme v Utility Room
            bool shouldBeDisabled = cameraPositionController.IsMoving || cameraPositionController.IsInUtilityRoom;
            
            if (shouldBeDisabled)
            {
                // Během přesunu nebo v Utility Room: zabarvi červeně a zablokuj
                if (!isButtonDisabledDuringMovement)
                {
                    openCamerasButtonImage.color = disabledButtonColor;
                    isButtonDisabledDuringMovement = true;
                }
            }
            else
            {
                // V Main Room (kanceláři) a neprobíhá přesun: vrať původní barvu a odblokuj
                if (isButtonDisabledDuringMovement && isPowerOn)
                {
                    openCamerasButtonImage.color = originalButtonColor;
                    isButtonDisabledDuringMovement = false;
                }
            }
        }
        ...
        {
            // KRITICKÉ: Zablokuj klávesové ovládání během přesunu nebo v Utility Room
            if (cameraPositionController != null && (cameraPositionController.IsMoving || cameraPositionController.IsInUtilityRoom))
            {
                return; // Klávesa nefunguje během přesunu nebo v Utility Room
            }
            ...
            // Kontrola Thatcher AI - pokud je Thatcher aktivní, kamery se nezapnou
            bool thatcherBlocking = false;
            if (thatcherAI == null)
            {
            ...
            if (thatcherAI != null)
            {
                thatcherBlocking = thatcherAI.IsThatcherBlockingCameras();
            }
            
            if (isPowerOn && !ventCamerasOpen && !thatcherBlocking)
            {
            ...
            else if (thatcherBlocking)
            {
                // Pokud Thatcher blokuje kamery, triggeruj smrt ale kamery se neotevřou
                if (thatcherAI != null)
                {
                    thatcherAI.TriggerDeathFromCameraAttempt();
                }
            }
        }

        // Klávesové ovládání kamer odstraněno - použij tlačítka na mapě
    }
    ...
    {
        // KRITICKÉ: Skryj tlačítko během přesunu mezi místnostmi
        if (cameraPositionController != null && cameraPositionController.IsMoving)
        {
            if (OpenCamerasButton != null)
            {
    ...
    {
        // KRITICKÉ: Zablokuj funkci během přesunu mezi místnostmi nebo v Utility Room
        if (cameraPositionController != null && (cameraPositionController.IsMoving || cameraPositionController.IsInUtilityRoom))
        {
            return; // Tlačítko nefunguje během přesunu nebo v Utility Room
        }
        ...
        // Kontrola Thatcher AI - pokud je Thatcher aktivní, kamery se nezapnou ale triggeruje se smrt
        bool thatcherBlocking = false;
        if (thatcherAI == null)
        {
            thatcherAI = FindFirstObjectByType<ThatcherAI>();
    ...
    public void OnCloseCamerasButtonClick()
{
    ...
    // Public metoda pro získání aktuálního indexu kamery (pro AraniAI)
